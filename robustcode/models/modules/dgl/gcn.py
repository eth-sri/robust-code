...
class GraphConv(nn.Module):
    ...
    def __init__(
        self, in_feats, out_feats, norm="both", weight=True, bias=True, activation=None
    ):
...
class GCNNet(GraphModel):
    @staticmethod
    def args():
        parser = argparse.ArgumentParser("GCN", add_help=False)
        parser.add_argument("--max_depth", type=int, default=8)
        return parser

    def __init__(self, config, d_in_vocabs, d_out_vocab, **kwargs):
        super(GCNNet, self).__init__(config, d_in_vocabs, d_out_vocab)

        self.layers = nn.Sequential(
            *[
                GraphConv(config.d_model, config.d_model)
