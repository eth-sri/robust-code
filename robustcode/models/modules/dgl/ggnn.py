...
class GGNNNet(GraphModel):
    @staticmethod
    def args():
        parser = argparse.ArgumentParser("GGNN", add_help=False)
        parser.add_argument("--max_depth", type=int, default=8)
        parser.add_argument(
            "--edge_types",
            type=boolean_string,
            default=True,
            help="Whether to include learnable weights per edge type",
        )
        return parser

    def __init__(self, config, d_in_vocabs, d_out_vocab, num_edge_types=None, **kwargs):
        super(GGNNNet, self).__init__(config, d_in_vocabs, d_out_vocab)
        assert num_edge_types is not None

        self.layer = GatedGraphConv(
            config.d_model,
            config.d_model,
            n_steps=config.max_depth,
            n_etypes=num_edge_types,
        )
        self.allow_edge_types = config.edge_types

    def forward_with_embed(self, embed, g: dgl.DGLGraph):
        edge_types = g.edata["type"]
        if not self.allow_edge_types:
            edge_types = th.ones_like(g.edata["type"])
        return self.layer(g, embed, edge_types)
